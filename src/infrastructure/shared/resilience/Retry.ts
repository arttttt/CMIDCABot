...
export class Retry {
   ...
   */
  static calculateBackoff(attempt: number, baseDelayMs: number, maxDelayMs?: number): number {
  ...
  static async withRetry<T>(
    ...
    shouldRetry: (error: unknown) => boolean = Retry.isRateLimitError,
  ): Promise<T> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        // Exponential backoff
        const delayMs = this.calculateBackoff(attempt, baseDelayMs);
  ...
  static async pollWithBackoff<T>(
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      const delayMs = Math.min(this.calculateBackoff(attempt, baseDelayMs, maxDelayMs), remaining);
  ...
  static async tryWithRetry<T>(
    ...
    try {
      return await this.withRetry(fn, maxRetries, baseDelayMs, () => true);
    } catch (error) {
      onError(error);
      return undefined;
    }
