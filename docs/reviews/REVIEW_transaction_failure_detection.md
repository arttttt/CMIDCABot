...
- [ ] [S1] Переименовать `"pending"` → `"timeout"` и обновить UI-сообщение
...
**🟡 Approved with comments** — реализация корректна, архитектура соблюдена. Рекомендуется исправить [S1] для улучшения UX.
