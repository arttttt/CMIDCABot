...
- [x] При падении транзакции на блокчейне (`status.err !== null`) пользователь видит сообщение об ошибке, а не "Purchase Complete"
- [x] Сообщение об ошибке содержит ссылку на Solscan для диагностики
- [x] Pending-статус показывается только при timeout (транзакция не подтверждена, но и не упала)
- [x] Логи содержат информацию о причине падения транзакции
